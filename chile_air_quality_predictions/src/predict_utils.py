...
def predict_on_rm(
    ...
    """
    geo_json_path = path.join("./data/input", "r13.geojson")
    with open(geo_json_path) as f:
        js = json.load(f)

    # Check each polygon to see if it contains the point
    polygons = []
    for map_feature in js["features"]:
        map_shape = shape(map_feature["geometry"])
        polygons.append(Polygon(map_shape))
    merged_polys = unary_union(polygons)

    (lon_min, lat_min, lon_max, lat_max) = merged_polys.bounds
    ...
    for i in range(n_points):
        ...
        for j in range(n_points):
            if merged_polys.contains(Point(lon_values[j], lat_values[i])):
...
def create_heat_map(
    ...
    map_hooray = folium.Map(location=[lat_center, lon_center], zoom_start=10)
    ...
    for index, station in df_day.iterrows():
        ...
        if imputed_col in station and isinstance(station[imputed_col], str):
            bg_color = "black"
            interpolated = "\nestimated"
        else:
            bg_color = "white"
            ...
            if len(df_day_no_station) >= k:
                ...
                if len(predictions) == 0:
                    ...
                    predictions = np.concatenate(
                        (
                            predictions,
                            np.array([df_day_station[target_pollutant].values, prediction[:, 0]]).T,
                        ),
                        axis=0,
                    )
...
def create_animation_features(
    ...
    for timestamp in unique_dates:
        ...
        predictions_xy, dlat, dlon = predict_on_rm(k_neighbors_model, n_points)
...
def create_circle(
    p: List[float], day_hour: datetime, radius: float, pollutant: str, color: Optional[str] = None
) -> Dict[str, Any]:
